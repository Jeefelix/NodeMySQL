...
connection.connect((err) => {
    ...
    fetchFromAPI((err,data) => {
        ...
        insertMySQL(connection, data.bpi, (err, results, fields)=> {
            if (err) throw err;
            console.log('successful ${results.affectedRows} doc');

            connection.query('SELECT * FROM wallet ORDER BY wallet DESC LIMIT 0,1', (err,results,fields)=> {
                if (err) throw err;
                console.log('the one month is ${results[0].coinvalue} and was ${results[0].valuedate}')
